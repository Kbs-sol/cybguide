...
// AI Assistant endpoint with search integration
apiRoutes.post('/ai-assistant', async (c) => {
  try {
    const { query, userEmail, interface: interfaceType } = await c.req.json()
    ...
    // Log the AI query
    await logUserAction(c, userEmail || 'anonymous', 'ai_query', { query, interface: interfaceType })
    ...
    try {
      searchResults = await searchForRelevantInfo(query, c.env.SEARCH_API_KEY, c.env.SEARCH_ENGINE_ID);
    } catch (searchError) {
      console.log('Search unavailable, using built-in responses');
    }
    ...
    return c.json({ 
      ...
      responseTime,
      model: response.model || 'built-in',
      timestamp: new Date().toISOString(),
      disclaimer: interfaceType === 'terminal' ? 
        "[SECURITY] Lab environments only. Unauthorized access is illegal." :
        "Remember: Use this knowledge only in authorized lab environments. Never target systems without explicit permission."
    })
    
  } catch (error) {
    console.error('AI Assistant error:', error)
    return c.json({ error: 'AI Assistant unavailable', details: error.message }, 500)
  }
})
...
// Search for relevant information from Reddit and forums
async function searchForRelevantInfo(query: string, searchApiKey?: string, searchEngineId?: string) {
  if (!searchApiKey || !searchEngineId) {
    return null;
  }
  
  try {
    // Construct search query focusing on cybersecurity forums and Reddit
    const searchQuery = `${query} site:reddit.com/r/cybersecurity OR site:reddit.com/r/netsec OR site:stackoverflow.com OR site:security.stackexchange.com`;
    
    const response = await fetch(
      `https://www.googleapis.com/customsearch/v1?key=${searchApiKey}&cx=${searchEngineId}&q=${encodeURIComponent(searchQuery)}&num=5`
    );
...
// Enhanced AI response with multiple model support and search context
async function generateEnhancedAIResponse(
  query: string, 
  searchResults: any,
  apiKeys: { openai?: string, openrouter?: string, groq?: string, gemini?: string },
  ...
  if (apiKeys.openrouter) {
    try {
      const result = await callOpenRouter(query, searchResults, apiKeys.openrouter, interfaceType);
      return { text: result, model: 'OpenRouter' };
    } catch (error) {
      console.log('OpenRouter failed, trying next model');
    }
  }
  
  if (apiKeys.groq) {
    try {
      const result = await callGroq(query, searchResults, apiKeys.groq, interfaceType);
      return { text: result, model: 'Groq' };
    } catch (error) {
      console.log('Groq failed, trying next model');
    }
  }
  
  if (apiKeys.gemini) {
    try {
      const result = await callGemini(query, searchResults, apiKeys.gemini, interfaceType);
      return { text: result, model: 'Gemini' };
    } catch (error) {
      console.log('Gemini failed, trying next model');
    }
  }
  
  if (apiKeys.openai) {
    try {
      const result = await callOpenAI(query, searchResults, apiKeys.openai, interfaceType);
      return { text: result, model: 'OpenAI' };
    } catch (error) {
      console.log('OpenAI failed, using built-in responses');
    }
  }
  
  // Fallback to enhanced built-in responses
  const builtInResponse = await generateAIResponse(query, undefined, searchResults, interfaceType);
  return { text: builtInResponse, model: 'Built-in Enhanced' };
}

// OpenRouter API integration
async function callOpenRouter(query: string, searchResults: any, apiKey: string, interfaceType?: string): Promise<string> {
  const context = searchResults ? buildSearchContext(searchResults) : '';
  const systemPrompt = buildSystemPrompt(interfaceType);
...
async function callGroq(query: string, searchResults: any, apiKey: string, interfaceType?: string): Promise<string> {
  const context = searchResults ? buildSearchContext(searchResults) : '';
  const systemPrompt = buildSystemPrompt(interfaceType);
...
async function callGemini(query: string, searchResults: any, apiKey: string, interfaceType?: string): Promise<string> {
  const context = searchResults ? buildSearchContext(searchResults) : '';
  const systemPrompt = buildSystemPrompt(interfaceType);
...
async function callOpenAI(query: string, searchResults: any, apiKey: string, interfaceType?: string): Promise<string> {
  const context = searchResults ? buildSearchContext(searchResults) : '';
  const systemPrompt = buildSystemPrompt(interfaceType);
...
// Build system prompt based on interface type
function buildSystemPrompt(interfaceType?: string): string {
  const basePrompt = `You are DEKE-AI, an ethical cybersecurity assistant inspired by Deke Shaw's analytical and calm personality. You provide guidance on cybersecurity topics while ensuring all advice is ethical and legal.`;
  
  if (interfaceType === 'terminal') {
    return `${basePrompt}

You are responding in a terminal interface. Keep responses technical and concise, but helpful. Use a professional hacker tone but emphasize ethical boundaries. Include practical commands and examples where appropriate. Always remind users to only practice on authorized systems.`;
  }
  
  return `${basePrompt}

Provide comprehensive, educational responses about cybersecurity. Always emphasize ethical practices and legal boundaries. Include practical examples and learning resources. Format responses clearly with headers and code examples where appropriate.`;
}

// Build context from search results
function buildSearchContext(searchResults: any): string {
  if (!searchResults?.items?.length) {
    return '';
  }
  
  const contextItems = searchResults.items.slice(0, 3).map((item: any) => 
    `Title: ${item.title}\nSnippet: ${item.snippet}\nURL: ${item.link}`
  ).join('\n\n');
  
  return `Recent discussions and information from cybersecurity communities:\n\n${contextItems}\n\nBased on this context and your cybersecurity expertise, please provide a comprehensive answer.`;
}

// Helper function to simulate AI response with enhanced Deke Shaw personality
async function generateAIResponse(query: string, apiKey?: string, searchResults?: any, interfaceType?: string): Promise<string> {
